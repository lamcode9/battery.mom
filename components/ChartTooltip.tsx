...
export function ChartTooltipPanel({
  ...
  label?: ReactNode
  items?: Array<Pick<ChartTooltipItem, 'name' | 'value' | 'color'> & { marker?: ReactNode }>
  total?: ReactNode
  children?: ReactNode
  className?: string
}) {
...
export function ChartTooltip({
  ...
  swatch?: (item: ChartTooltipItem) => ReactNode
}) {
  ...
  return (
    <ChartTooltipPanel
      label={heading}
      items={swatch ? items.map(item => ({ ...item, marker: swatch(item) })) : items}
      total={total}
    />
  )
}
