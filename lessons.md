...
- Recharts `AreaChart` only draws `Area`. A `Line` child is dropped unless the chart is a `ComposedChart`. The generation stack does not add a boundary hairline, margin label, or wash: nine source hues are already the chart, and Fossil, Renewable, and Nuclear are named once in the legend under it. Coal, natural gas, and oil carry a smooth fractal-noise grain (`generation-grain.tsx`) clipped to the shape. The filter region has to be the object box, and `feComposite` `in` has to drop everything outside the fill: a user-space rectangle (the old 1600×480 region) paints that noise across the plot, the margins, and the renewable bands. Constant mid-gray only varies alpha, so overlay of it does not texture an opaque fill and the leftover noise alpha is a halo. Write the noise into the color, then clip. Tooltip dots for those three sources sit in an HTML `overflow-hidden` circle, because an SVG filter still escapes the circle edge. A speck pattern needs a viewBox or the marks do not repeat, and a small tile still reads as a grid. Hydro, bioenergy, solar, wind, other renewables, and nuclear stay flat fills. Do not put that grain on the state-of-battery-power chart, which is a different two-series figure. Area animation reveals the stack with a left-to-right clip, so a shot taken mid-animation looks like the series ends early. The generation areas set `isAnimationActive={false}` so 2015–2025 is painted in full on first draw.
