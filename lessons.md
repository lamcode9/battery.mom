...
- Recharts `AreaChart` only draws `Area`. A `Line` child is dropped unless the chart is a `ComposedChart`. The generation stack does not add a boundary hairline: nine source hues are already the chart, and Fossil, Renewable, and Nuclear are named once in the legend under it. If a boundary series is ever drawn, keep it out of the tooltip (`tooltipType="none"` plus a dataKey filter) or the cumulative value lands in the total.
