...
- Home scenario comparison: the 20-year cost chart's Grid only bar is the household bill (`daily load × 365 × residential tariff × years`). It does not add EV charging or petrol. The other bars are equipment plus electricity still bought from the grid, at today's tariff held flat. The red/green "cost vs savings" chart is a separate question.
- Chart hover cards use `ChartHoverTooltip` / `ChartTooltipPanel` from `components/ChartTooltip.tsx` (the state-of-battery-power hover). Do not restyle Recharts' default tooltip per chart. Skip static labeled figures (sparklines, LabelList bars). Heatmaps and share bars that are not Recharts use `AnchoredChartTooltip` so the card is not clipped. A wrapper around Recharts `<Tooltip />` must keep `displayName = 'Tooltip'` or `findChildByType` drops it and the chart has no hover at all.
- Recharts `AreaChart` only draws `Area`. A `Line` child is dropped unless the chart is a `ComposedChart`. The generation stack does not add a boundary hairline, margin label, or wash: nine source hues are already the chart, and Fossil, Renewable, and Nuclear are named once in the legend under it. Coal, natural gas, and oil carry the original same-hue fractal grain (`generation-grain.tsx`): collapse the noise to one luminance, mix a little of it into the fill (`k3` around 0.4, up from the first 0.2), then `feComposite` `in`. Feeding red, green, and blue noise through separately turns the bands into confetti. The filter region is the shape's box, not a plot-sized rectangle. Tooltip dots for those three sources use that same mix inside an HTML `overflow-hidden` circle, passed as the generation chart's `swatch`. A speck pattern needs a viewBox or the marks do not repeat, and a small tile still reads as a grid. Hydro, bioenergy, solar, wind, other renewables, and nuclear stay flat fills. Do not put that grain on the state-of-battery-power chart, which is a different two-series figure. Area animation reveals the stack with a left-to-right clip, so a shot taken mid-animation looks like the series ends early. The generation areas set `isAnimationActive={false}` so 2015–2025 is painted in full on first draw.
...
- Do not publish a named customer quote without a source you can cite (interview, submitted story with permission, or a published installer case). The three quotes on `/bess/case-studies` were written into the page on 26 Feb 2026 and matched no other site. Remove that speech rather than inventing a citation.
- Do not call an unsourced sketch a case study. `/bess/case-studies` stays at that URL, but the page and nav say Scenarios. The commercial calculator cards are Illustrations. A case study needs an owner, a system, a measured before and after, and a source.
